...
public class ServeurBanque extends Serveur {
    ...
    public void supprimeInactifs() {
        long tempsActuel = System.currentTimeMillis(); //temps act.
        ListIterator<Connexion> it = connectes.listIterator();  // Utilisation d'un iterator pour parcourir la liste des connexions

        // Parcourir toutes les connexions
        while (it.hasNext()) {
            ConnexionBanque connexion = (ConnexionBanque) it.next();

            // Vérifier si la connexion est inactive depuis plus que le délai d'inactivité
            if (tempsActuel - connexion.getTempsDerniereOperation() > DELAI_INACTIVITE) {
                // 1. Envoyer "END" au client
                connexion.envoyer("END");

                // 2. Fermer la connexion
                connexion.close();

                // 3. Enlever la connexion de la liste des connectés
                it.remove();
